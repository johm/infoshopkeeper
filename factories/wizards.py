...
class makepdf(factory):
    def __init__(self):
        factory.__init__(self, ["popups.genpdf", "GenPdfPopup"])

class select:
    def GenerateOnPress(self, frame_object,label):
        return lambda event : self.do_easyselect_wizard(frame_object,event,label)
    
    def do_easyselect_wizard(self, frame_object,event,label):
...
class editions:
    def GenerateOnPress(self, frame_object,label):
        return lambda event : self.do_edition_wizard(frame_object,event,label)
