...
if len(sys.argv) == 3: 
	date1=sys.argv[1]
	date2=sys.argv[2]		
	cursor.execute("SELECT * from transactionLog where action='TAX' and date>=%s and date<=ADDDATE(%s,INTERVAL 1 DAY)  order by date",(date1,date2))
else:
	if len(sys.argv) == 4:
		date1=sys.argv[2]
		date2=sys.argv[3]		
		cursor.execute("SELECT * from transactionLog where action='TAX' and info LIKE %s and date>=%s and date<=ADDDATE(%s,INTERVAL 1 DAY)  order by date",("%"+sys.argv[1]+"%",date1,date2))
	else:
