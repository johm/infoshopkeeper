...
<div class="same_author" style="float:right;width:250px;z-index:1;">
<h3>Filed under: </h3>
<ul>
   <li py:for="s in list(thetitle.sections)">
     <a href="/inventory/section/${s.id}">${s.sectionName}</a>
  </li>
</ul>
...
<a href="/inventory/private/title/${thetitle.id}" target="_blank">[Edit]</a>
</span>
...
<h2>Red Emma's Price: $$${thetitle.highest_price_book().listprice}</h2>
       <img src="http://covers.openlibrary.org/b/isbn/${thetitle.isbn}-M.jpg"  style="margin:5px;border:1px solid #990000;"/>
<!--
