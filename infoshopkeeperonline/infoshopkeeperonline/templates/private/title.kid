...
<a href="https://store.redemmas.org/titleedit?id=${thetitle.id}">edit</a>
<h2>Ordered from: 
...
${["Confirmed %s " % t.when_tagged for t in book.tags if t.tagkey=='confirmation11' and t.tagcategory.description=='inventory']}
<a py:if="len([t for t in book.tags if t.tagkey=='confirmation11' and t.tagcategory.description=='inventory'])==0"  href="/private/confirm?book=${book.id};title=${thetitle.id}">CONFIRM</a></span>
<a href="https://store.redemmas.org/bookedit?id=${book.id}" target="_blank">+</a>
</li>
</ul>
</div>
<div class="titleform">${titleform(action="/inventory/private/edit_title",value=dict(title_id=thetitle.id,preferred_distributor=thetitle.get_unique_tag(category='distribution',key="preferred"),sections=titlesections))}	
</div>
<!--<div class="titlepersistenceform">{titlepersistenceform(action="/private/persist",value=dict(sections=persisting_sections))}	
</div>-->
</body>
</html>
